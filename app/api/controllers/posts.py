...
async def get_post_info(db: Session, skip: int, limit: int):
    post = get_all_post_info(db, skip, limit)
    return post if post else []



async def get_my_posts(db: Session, user_id: int, skip: int, limit: int, post_status: str | None = None):
    """
...
async def get_post_info_with_slug(db: Session, slug: str, skip: int, limit: int):
